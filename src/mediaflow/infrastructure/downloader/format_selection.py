...
def build_format_selection(preset: DownloadPreset) -> YtDlpFormatSelection:
    ...
    combined_filter = f"{quality_filter}[vcodec!=none][acodec!=none]"
    base_selector = f"bestvideo{quality_filter}+bestaudio/best{combined_filter}"
    selector = base_selector
    if preset.preferred_frames_per_second is not None:
        fps_filter = f"[fps={preset.preferred_frames_per_second}]"
        preferred = (
            f"bestvideo{quality_filter}{fps_filter}+bestaudio/"
